...
{
	SpineFace *pRet = new(std::nothrow) SpineFace(rSFileName, scale);
	if (pRet && pRet->init(rSFileName))
	{
		pRet->autorelease();
		return pRet;
	}
 ...
 SpineFace::SpineFace(const std::string &rSFileName, float scale /*= 1*/)
	 : mySpine(create_spSkeletonData_WithFile(rSFileName, scale), false)
 {

 }

//在子类 init 的时候首先要初始化 Spine Face
bool SpineFace::init( const std::string &rSFileName)
{
	bool bRet = false;
	do
	{
...
float SpineFace::playOnce(const std::string& name, const OnPlayOnceListener& listener, float timescale /*= 1*/)
{
	...
	setTrackCompleteListener(trackEntry, [=](int trackIndex, int loopCount){
		if (listener){
			listener(trackIndex);
		}
	});
...
spSkeletonData* SpineFace::create_spSkeletonData_WithFile(const std::string& skeletonDataFile, float scale /*= 1*/)
{
	...
	if (isExist){
		...
		json->scale = scale;
		skeletonData = spSkeletonJson_readSkeletonDataFile(json, jsonFile.c_str());
